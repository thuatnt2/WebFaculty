...
$year = $_GET['year'];
$sql_query = @mysql_query("SELECT * FROM tbl_thongbaogv WHERE Year(`date`) = {$year}'");
...
<div id="content-blog">
    ...
    $sql = "SELECT * FROM tbl_thongbaogv WHERE idGV = {$idGV} AND Year(`date`) = {$year} ORDER BY date DESC LIMIT 0,10";
    ...
    while ($row = mysql_fetch_array($result)) {
      ...
      //  $tomtat=count($nd);
        ...
        if($demnd > 10){
            $output .='<p style="font-weight: bold; color: blue;">Ngày đăng: '.$inngay. '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Môn học: <a href="?mod=danhmuctin&idGV='. $idGV .'&idLoai='.$id_theloai.'">'.$loai1['tenLoai'].'</a><p>';
    ...
    ?>
    <?php
            echo $output;    $total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_thongbaogv WHERE idGV = {$idGV} AND  Year(`date`) = {$year}"),0);  
