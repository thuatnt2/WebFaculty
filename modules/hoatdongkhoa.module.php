...
<div class="contain_ndtin">Hoạt Động Khoa</div>
...
//phan trang   
    ...
    if (isset($_GET['page']) && (int) $_GET['page']) {
        ...
        $query = "SELECT COUNT(id_tintuc) FROM tbltintuc WHERE id_theloai='3'";
...
<div>
    <ul class="nav1">
        ...
        if ($page > 1) {
            ...
            //hien? thi tran prev
            if ($current != 1) {
                echo"<li><a href='?mod=hoatdongkhoa&id_theloai=$id_theloai&start=$prev'>Prev</a></li>";
            }
            // hien thi so link
            for ($i = 1; $i <= $page; $i++) {
                if ($current != $i) {
                    echo "<li><a href='?mod=hoatdongkhoa&id_theloai=$id_theloai&start=" . ($display * ($i - 1)) . "'>$i</a></li>";
                } else {
                    echo"<li>$i</li>";
                }
            }
