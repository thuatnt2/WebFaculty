...
<div id="content-blog">
    ...
    $sql = "SELECT * FROM tbl_thongbaogv WHERE idGV = {$idGV}  ORDER BY date DESC LIMIT 0,10";
    $result = mysql_query($sql);

    while ($row = mysql_fetch_array($result)) {
        $sql_query = @mysql_query("SELECT * FROM tbl_theloaitbgv WHERE idLoai='{$row['idLoai']}'");
      ...
      //  $tomtat=count($nd);
        ...
        if($demnd > 50){
            ...
            $output .='<div class="float">' . noidungtt(50, $nd) ;
    ...
    <?php
    echo $output;
    $total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_thongbaogv WHERE idGV = {$idGV}"),0);  
