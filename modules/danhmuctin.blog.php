...
$idLoai = $_GET['idLoai'];
$sql_query = @mysql_query("SELECT * FROM tbl_theloaitbgv WHERE idLoai='{$idLoai}'");
$loai = @mysql_fetch_array( $sql_query ); 
?>
<div id="content-blog">
    <div class="tinmoinhat">Tin: <?php echo $loai['tenLoai'];  ?></div>
    <?php
    if(!isset($_GET['page'])){  
        $page = 1;  
        } else {  
    ...
    $sql = "SELECT * FROM tbl_thongbaogv WHERE idGV = {$idGV} AND idLoai = {$idLoai} ORDER BY date DESC LIMIT 0,10";
    ...
    ?>
    <?php
            echo $output;    $total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_thongbaogv WHERE idGV = {$idGV} AND idLoai = {$idLoai}"),0);  
